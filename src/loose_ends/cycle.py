...
from loose_ends.discovery import Classifier, Message, aggregate_signals
...
def run_cycle(ledger: JsonLedger, estate_id: str, messages: list[Message], classify: Classifier,
              model: Model, mailer: Mailer, playbooks: dict[str, Playbook], today: date) -> CycleReport:
    report = CycleReport()
    ...
    if new:
        ...
        for account in aggregate_signals(classify(new)):
    ...
    d = dispatch(ledger, mailer, model, estate_id, playbooks, today)
    f = follow_up(ledger, mailer, model, estate_id, playbooks, today)
